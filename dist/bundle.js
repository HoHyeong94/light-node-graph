(function (global) {
  ...
  function DYXbeam4(iPoint, jPoint, iSectionPoint, jSectionPoint, xbeamSection) {
    ...
    let lstiff2 = PlateRestPoint({ x: bl.x, y: bl.y + xs.lflangeHeight + xs.webHeight + xs.flangeThickness }, tl, lGradient, 0, xs.stiffWidth);
    ...
    ToGlobalPoint(centerPoint, lwebPlate[1]),
    ToGlobalPoint(centerPoint, rwebPlate[1])];
    for (let i = 0; i < 4; i++) {
      let sign = i % 2 === 0 ? 1 : -1;
      let grad = lRad;
  ...
  LineViewer.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(mesh, [2, "line","total"]);
  };
  ...
  SteelPlateView.prototype.onExecute = function() {
    ...
    global.sceneAdder(group, [0, "steelBox","total"]);
  };
  ...
  function HorBracingView(){
    ...
    DeckView.prototype.onExecute = function() {
      ...
      // },"deck"); 
      global.sceneAdder(DeckPointView(this.getInputData(0),this.getInputData(1),this.getInputData(2)), [0, "deck", "total"]); 
    };
    ...
    StudView.prototype.onExecute = function() {
      ...
      // },"stud"); 
      global.sceneAdder( StudMeshView(this.getInputData(0),this.getInputData(1)), [0, "stud", "total"]); 
    };
  ...
  AnalysisView.prototype.onExecute = function () {
    ...
    global.sceneAdder(AnalysisModel(this.getInputData(0),this.getInputData(1)),[2, "analysis", "total"]);
  };
  ...
  TopViewer.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group,[6, "topView", "total"]);
  };
  ...
  GirderGeneralView1.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group, [this.getInputData(1), "GirderGeneralView1", "total"] );
  };
  ...
  GirderGeneralView2.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group, [this.getInputData(4), "GirderGeneralView2", "total"]);
  };
  ...
  LineDraw.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group, [3, "LineView", "total"]);
  };
  ...
  LineSideDraw.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group, [4, "LineSideView", "total"]);
  };
  ...
  GirderLayoutDraw.prototype.on3DExecute = function() {
    ...
    global.sceneAdder(group.plan, [3, "GirderLayout", "total"]);
    global.sceneAdder(group.side, [4, "GirderLayoutSide", "total"]);
  };
  ...
  function CompositeFrameGen(nodeNumDict, frameInput, deckLineDict, sectionPointDict, gridPoint, slabInfo) { //gridModelData, xbeamData, 
      ...
      for (let j = 0; j < gridModelL[0].length + 1; j++) { //모델에 따라서 격자가 서로 다를 수 있음. 개수가 동일하지 않음, 추후 수정이 필요
          ...
          for (let i = 0; i < gridModelL.length; i++) {
              ...
              if (i < gridModelL.length - 1) {
                  if (j === 0) {
                      inode2 = "LD" + gridModelL[i+1][j].substr(2);
                      jnode2 = gridModelL[i+1][j];
                      ipoint2 = deckLineDict[0].find(elem => elem.key === inode2).point;
                      jpoint2 = gridPoint[jnode2];
                  } else if (j === gridModelL[i].length) {
                      inode2 = gridModelL[i+1][j - 1];
                      jnode2 = "RD" + gridModelL[i+1][j - 1].substr(2);
                      ipoint2 = gridPoint[inode2];
                      jpoint2 = deckLineDict[1].find(elem => elem.key === jnode2).point;
                  } else {
                      inode2 = gridModelL[i+1][j - 1];
                      jnode2 = gridModelL[i+1][j];
                      ipoint2 = gridPoint[inode2];
                  ...
                  ivecF = [ipoint2.x - ipoint.x, ipoint2.y - ipoint.y];
                  jvecF = [jpoint2.x - jpoint.x, jpoint2.y - jpoint.y];
              } else {
                  ivecF = [0, 0, 0];
                  jvecF = [0, 0, 0];
              }
