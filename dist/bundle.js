(function (global) {
    ...
    * @returns {any}
    */
    function LineGen(val) {
        try {
            const ret = wasm.LineGen(addBorrowedObject(val));
    ...
    * @returns {any}
    */
    function LineOffset(val, offset) {
        try {
            const ret = wasm.LineOffset(addBorrowedObject(val), offset);
            return takeObject(ret);
        } finally {
            heap[stack_pointer++] = undefined;
        }
    ...
    Initialize().then(() => {
        ...
        global.LiteGraph.registerNodeType("nexivil/LineGenerator", LineGenerator);

        function LineOffset$1() {
            this.addInputs([
                ["Line", "line"],
                ["offset", "number"]
            ]);
        ...
        LineOffset$1.prototype.onExecute = function () {
            let a = this.getInputData(0);
            let b = this.getInputData(1);

            let r = LineOffset(a, b);
            this.setOutputData(0, r);
        };

        global.LiteGraph.registerNodeType("nexivil/LineOffset", LineOffset$1);
